...
import diffae.src.handlers.utils as handlers_utils
import torch
...
"""
RUN AS :
python -m src.training_scripts.encodec_hdae --batch_size 128 --log_freq 2 --valid_freq 10 --save_freq 50 --config encodec_hdae_02_V2
"""
...
def main(args):
    ...
    #### Process Handler
    hierarchical_dae = HierarchicalDAE(
    ...
    if LOAD_MODEL:
        if LOAD_EPOCH is not None:
            hierarchical_dae.load(MODEL_PATH, extension=f"epoch_{LOAD_EPOCH}")
        else:
            hierarchical_dae.load(MODEL_PATH)

    # Training start infos
    print("============================================")
    print("--------------- Model Infos ----------------")
    print(
        f"Diffusion Unet: {handlers_utils.get_number_params(hierarchical_dae.diffusion_unet)} M Params"
    )
    print(
        f"Semantic Encoder: {handlers_utils.get_number_params(hierarchical_dae.semantic_encoder)} M Params"
    )
    print("============================================")

    hierarchical_dae.train(
        n_epochs=EPOCHS,
        log_freq=LOG_FREQ,
