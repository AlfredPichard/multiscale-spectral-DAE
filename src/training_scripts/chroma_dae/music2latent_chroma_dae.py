...
"""
RUN AS :
python -m diffae.src.training_scripts.chroma_diffae.music2latent_chroma_dae --batch_size 16 --log_freq 1 --valid_freq 2 --save_freq 5 --stage 1 --config music2latent_mel_hdae_02 --dataset slakh
"""
...
def run(args):
    ...
    MODEL_PATH = f"diffae/src/saved_models/{DATASET}:{CONFIG['name']}"
    LOG_DIR = f"diffae/src/logs/{DATASET}:{CONFIG['name']}:stage_{STAGE}"
